...
def generate_pseudo(model, target_loader, save_dir, n_class=4, pseudo_dict=dict(), logger=None):
    ...
    for image, labels in tqdm(target_loader):
        ...
        for _i, fname in enumerate(labels['fname']):
            ...
            imsave(os.path.join(save_dir, 'pred', fname), label.astype(np.uint8), check_contrast=False)
